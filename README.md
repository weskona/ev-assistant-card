...
- **Inline confirmation form:** when a Fremdladung is pending, the detail view shows an editable kWh/price form with Save/Discard buttons that call the integration's services directly (`config_entry_id` is resolved automatically from the card's device — works correctly even if you run more than one EV Assistant instance).
