...
- **Status header:** the detail view opens with an animated ring showing charge-efficiency (measured or manual fallback) plus pill badges for "charge open/OK" and "trip open/OK". If a wallbox energy meter is configured, kWh/cost are shown as a Fremdladung-vs-Heimladen bar comparison instead of flat tiles.
- **Inline confirmation form:** when a Fremdladung is pending, the detail view shows an editable kWh/price form with Save/Discard buttons that call the integration's services directly (`config_entry_id` is resolved automatically from the card's device — works correctly even if you run more than one EV Assistant instance). Discard asks for an inline "really discard?" confirmation first, since it throws away the detected estimate irreversibly. **More than one charge can be pending at once** (e.g. two charging stops before you get around to confirming either) — the card shows a separate form for each and confirms/discards the correct one.
...
- **Statuskopf:** die Detailansicht startet mit einem animierten Ring für den Ladewirkungsgrad (gemessen oder manueller Fallback) sowie Pill-Badges für „Ladung offen/OK" und „Fahrt offen/OK". Ist ein Wallbox-Energiezähler konfiguriert, werden kWh/Kosten als Fremdladung-vs-Heimladen-Balkenvergleich statt als flache Kacheln angezeigt.
- **Direktes Erfassungsformular:** ist eine Fremdladung offen, zeigt die Detailansicht ein bearbeitbares kWh/Preis-Formular mit Speichern/Verwerfen-Buttons, die die Services der Integration direkt aufrufen (`config_entry_id` wird automatisch aus dem Geräte-Bezug der Karte ermittelt — funktioniert auch korrekt bei mehreren EV-Assistant-Instanzen). Verwerfen fragt vorher inline "wirklich verwerfen?" nach, da die erkannte Schätzung dabei unwiderruflich verloren geht. **Es können mehrere Ladungen gleichzeitig offen sein** (z.B. zwei Ladestopps, bevor du zum Bestätigen kommst) — die Karte zeigt dann ein eigenes Formular je Ladung und bestätigt/verwirft die richtige.
...
- **Fahrtenbuch:** ist eine Fahrt erkannt (setzt EV Assistant v0.14.0+ mit konfigurierter Kilometerstand-Entität voraus), zeigt die Detailansicht neben dem Ladungs-Formular ein eigenes Start-/Zielort-Formular, das direkt `ev_assistant.log_trip`/`discard_pending_trip` aufruft. Eine eigene ausklappbare Historie listet die letzten 10 bestätigten Fahrten (Datum, km, Strecke), je mit Bearbeiten- (`edit_trip`) und Löschen-Button (`delete_trip`) — dasselbe Muster wie bei Fremdladungen. Bleibt komplett ausgeblendet, wenn das Fahrtenbuch-Feature in der zugrunde liegenden Integration nicht eingerichtet ist. Mit EV Assistants optionalem `gps_entity` (ab v0.15.0) werden die Ortsfelder aus dessen Zonen-Vorschlag vorausgefüllt — bleiben vor dem Bestätigen weiterhin frei editierbar.
